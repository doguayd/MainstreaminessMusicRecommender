...
# ============================================================
# PLOTS — save all figures to the same directory as this script
# ============================================================
...
# ----------------------------------------------------------------
# FIGURE 1 — AF vs AF-ILF top-5 artists for selected countries
# ----------------------------------------------------------------
selected = ["TR", "FI", "US", "GB"]
fig, axes = plt.subplots(len(selected), 2, figsize=(14, 10))
fig.suptitle("Figure 1 — Top-5 Artists: Raw AF vs AF-ILF\n(selected countries)",
...
for row, c in enumerate(selected):
    ...
    ax_af.barh(range(5), top_af.values[::-1], color=BLUE, alpha=0.85)
    ax_af.set_yticks(range(5))
    ax_af.set_yticklabels(top_af.index.tolist()[::-1], fontsize=9)
    ax_af.set_title(f"{c} — AF", fontsize=10, fontweight="bold")
    ax_af.set_xlabel("Playcount", fontsize=8)
    ax_af.xaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f"{x:,.0f}"))

    ax_if.barh(range(5), top_afilf.values[::-1], color=TEAL, alpha=0.85)
    ax_if.set_yticks(range(5))
    ax_if.set_yticklabels(top_afilf.index.tolist()[::-1], fontsize=9)
    ax_if.set_title(f"{c} — AF·ILF", fontsize=10, fontweight="bold")
    ax_if.set_xlabel("AF·ILF Score", fontsize=8)

plt.tight_layout()
savefig("fig1_af_vs_afilf.png")

# ----------------------------------------------------------------
# FIGURE 2 — Country mainstreaminess heatmap (6 key measures)
# ----------------------------------------------------------------
key6 = ["Fg:AF,u:AF", "Fc:AF,u:AF",
        "Dg:AF,u:AF", "Dc:AF,u:AF",
        "Cg:AF,u:AF", "Cc:AF,u:AF"]
hmap = country_avg[key6].sort_values("Cg:AF,u:AF", ascending=False)

fig, ax = plt.subplots(figsize=(10, 6))
im = ax.imshow(hmap.values, aspect="auto", cmap="YlOrRd")
ax.set_xticks(range(len(key6)))
ax.set_xticklabels(key6, rotation=30, ha="right", fontsize=9)
ax.set_yticks(range(len(hmap)))
ax.set_yticklabels(hmap.index, fontsize=10)
plt.colorbar(im, ax=ax, label="Mean mainstreaminess score")
# Annotate cells
for i in range(len(hmap)):
    for j in range(len(key6)):
        ax.text(j, i, f"{hmap.values[i, j]:.3f}",
                ha="center", va="center", fontsize=7,
                color="black" if hmap.values[i, j] < 0.6 else "white")
ax.set_title("Figure 2 — Country-Level Average Mainstreaminess Heatmap\n"
             "(sorted by Cg:AF — global Kendall tau)", fontsize=12, fontweight="bold")
plt.tight_layout()
savefig("fig2_country_heatmap.png")

# ----------------------------------------------------------------
# FIGURE 3 — Low / Mid / High distribution stacked bar chart
# ----------------------------------------------------------------
dist_sorted = dist.sort_values("Cg:AF,u:AF" if "Cg:AF,u:AF" in dist.columns
                               else "Total", ascending=True)
# Re-sort by total for readability
dist_sorted = dist[["Low", "Mid", "High"]].loc[
    country_avg["Cg:AF,u:AF"].sort_values(ascending=True).index
]

fig, ax = plt.subplots(figsize=(10, 6))
bottom = np.zeros(len(dist_sorted))
colors_lvl = {"Low": "#EF4444", "Mid": "#F59E0B", "High": "#10B981"}
for lvl in ["Low", "Mid", "High"]:
    vals = dist_sorted[lvl].values
    ax.barh(range(len(dist_sorted)), vals, left=bottom,
            label=lvl, color=colors_lvl[lvl], alpha=0.88)
...
ax.set_yticklabels(dist_sorted.index, fontsize=10)
ax.set_xlabel("Number of users", fontsize=10)
ax.legend(loc="lower right", fontsize=9)
ax.set_title("Figure 3 — Mainstreaminess Level Distribution by Country\n"
             "(sorted ascending by global Kendall tau)", fontsize=12, fontweight="bold")
plt.tight_layout()
savefig("fig3_level_distribution.png")

# ----------------------------------------------------------------
# FIGURE 4 — Global mainstreaminess ranking (horizontal bar)
# ----------------------------------------------------------------
rank_sorted = country_avg["Cg:AF,u:AF"].sort_values(ascending=True)
fig, ax = plt.subplots(figsize=(9, 6))
bars = ax.barh(range(len(rank_sorted)), rank_sorted.values,
               color=[TEAL if v >= 0 else ORANGE for v in rank_sorted.values],
               alpha=0.88)
ax.set_yticks(range(len(rank_sorted)))
ax.set_yticklabels(rank_sorted.index, fontsize=10)
ax.axvline(0, color="black", linewidth=0.8, linestyle="--")
for i, v in enumerate(rank_sorted.values):
    ax.text(v + 0.005 if v >= 0 else v - 0.005, i,
            f"{v:+.4f}", va="center",
            ha="left" if v >= 0 else "right", fontsize=8)
ax.set_xlabel("Average Cg:AF,u:AF (Kendall tau)", fontsize=10)
ax.set_title("Figure 4 — Global Mainstreaminess Ranking by Country\n"
             "(Cg:AF,u:AF — Kendall tau vs global population)",
             fontsize=12, fontweight="bold")
plt.tight_layout()
savefig("fig4_global_ranking.png")

# ----------------------------------------------------------------
# FIGURE 5 — Method comparison: Novelty & Personalization
# ----------------------------------------------------------------
methods   = ["Popularity\nBaseline", "User-KNN\n(k=10)", "SVD\n(20 factors)",
             "Proposed\n(MS-AF·ILF)"]
novelties = [pop_novelty, knn_novelty, svd_novelty, ms_novelty]
persons   = [pop_personalization, knn_personalization,
             svd_personalization, ms_personalization]

x   = np.arange(len(methods))
w   = 0.35
fig, ax = plt.subplots(figsize=(10, 5))
b1 = ax.bar(x - w/2, novelties, w, label="Novelty",        color=BLUE,   alpha=0.88)
b2 = ax.bar(x + w/2, persons,   w, label="Personalization", color=ORANGE, alpha=0.88)
...
ax.set_title("Figure 5 — Recommendation Method Comparison\n"
             "Novelty vs Personalization (EVAL_SAMPLE = 50 users)",
             fontsize=12, fontweight="bold")
plt.tight_layout()
savefig("fig5_method_comparison.png")
